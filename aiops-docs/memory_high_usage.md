...
> **适用场景**：课题组共用服务器、训练节点或容器；高内存常与大型模型、数据加载或 GPU 侧缓存相关，可结合 `gpu_high_usage.md` 排查。

## 与「206 服务器监控」页面对照（Prometheus）

- 对话或前端快照里的 **`memoryPercent`**：表示整机 RAM 已用比例（由 `node_memory_MemAvailable_bytes` / `MemTotal` 推算），与「内存占用如何」类问题直接对应。  
- 若快照中 `memoryPercentError` 非空，说明当前查不到数据，应先排查 exporter 与 `instance` 标签，不要凭感觉报百分比。  
- 处置流程仍以本文下文「排查步骤 / 常见原因」为准；总览字段说明见 `lab_server_snapshot_and_actions.md`。
