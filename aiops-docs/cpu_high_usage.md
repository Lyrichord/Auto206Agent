...
> **适用场景**：课题组共用服务器、计算节点或容器环境；与 GPU 高占用可交叉查看进程与作业调度（参见同目录 `gpu_high_usage.md`）。

## 与「206 服务器监控」页面对照（Prometheus）

- **`cpuPercent`**：整机 CPU 非 idle 估算占用；**`load1`**：1 分钟平均负载。回答「CPU/负载现在高不高」应引用这两项及时间戳。  
- 高 CPU 但负载不高：可能是短时突发；负载持续高于 CPU 核数： Runnable 排队多，按本文步骤查进程与 IO。  
- 字段释义汇总见 `lab_server_snapshot_and_actions.md`。
