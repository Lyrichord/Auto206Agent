...
- 快照 JSON 中的 **`diskRootPercent`**：表示根挂载点 `/` 的已用空间比例（与 node_exporter 文件系统指标一致）。用户问「硬盘是不是快满了」应优先引用该值及 `queriedAtText`。  
- **`diskRootPercentError`** 有内容时：可能未采集到 `/` 或标签不匹配，需检查 Prometheus 目标与 `mountpoint`、`fstype` 过滤。  
- **「该怎么办」**：在陈述当前占用后，按本文「常见原因分析」「处理方案」分场景执行；总览见 `lab_server_snapshot_and_actions.md`。
