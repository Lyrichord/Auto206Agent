...
# 默认目标：显示帮助信息
help:
	@echo "$(GREEN)Auto206Agent Makefile$(NC)"
	@echo ""
	@echo "可用命令："
	...
	@echo "  $(YELLOW)make upload$(NC)  - 递归上传 aiops-docs 下所有 .md（含 group/ 等子目录）"
...
init:
	@echo "$(GREEN)🚀 开始一键初始化 Auto206Agent...$(NC)"
	@echo ""
	...
	@$(MAKE) wait
	@echo ""
	@echo "$(YELLOW)步骤 4/4: 递归上传 aiops-docs（含 group/）到向量数据库$(NC)"
	@$(MAKE) upload
	@echo ""
	@echo "$(GREEN)═══════════════════════════════════════════════════════$(NC)"
...
# 等待服务器就绪（首次 mvn spring-boot:run + Milvus 索引起步较慢，适当放宽）
wait:
...
# 上传所有文档
upload:
	@echo "$(YELLOW)📤 开始递归上传 $(DOCS_DIR) 下所有 .md / .markdown...$(NC)"
	@if [ ! -d "$(DOCS_DIR)" ]; then \
	...
	success=0; \
	failed=0; \
	for file in $$(find $(DOCS_DIR) -type f \( -name '*.md' -o -name '*.markdown' \) 2>/dev/null | sort); do \
		if [ -f "$$file" ]; then \
			count=$$((count + 1)); \
			rel=$${file#$(DOCS_DIR)/}; \
			echo "$(YELLOW)  [$$count] 上传文件: $$rel$(NC)"; \
			...
			if [ "$$http_code" = "200" ]; then \
				echo "$(GREEN)      ✅ 成功: $$rel$(NC)"; \
				success=$$((success + 1)); \
			else \
				echo "$(RED)      ❌ 失败: $$rel (HTTP $$http_code)$(NC)"; \
				echo "$$body" | head -n 3; \
				failed=$$((failed + 1)); \
			fi; \
			sleep 1; \
...
# 显示文档列表
list-docs:
	@echo "$(YELLOW)📚 $(DOCS_DIR) 下所有 .md / .markdown（递归）:$(NC)"
	@if [ -d "$(DOCS_DIR)" ]; then \
		find $(DOCS_DIR) -type f \( -name '*.md' -o -name '*.markdown' \) 2>/dev/null | sort | while read -r f; do ls -lh "$$f"; done || echo "$(RED)没有找到 .md 文件$(NC)"; \
	else \
		echo "$(RED)目录 $(DOCS_DIR) 不存在$(NC)"; \
	fi
