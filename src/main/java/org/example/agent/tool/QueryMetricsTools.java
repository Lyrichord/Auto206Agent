...
public class QueryMetricsTools {
    ...
    /** 真实 Prometheus 不可达或 API 非 success 时，仍返回演示告警 JSON，避免 AI Ops 无输入 */
    @Value("${prometheus.fallback-mock-on-error:true}")
    ...
    public void init() {
        this.httpClient = new OkHttpClient.Builder()
                ...
                .build();
        logger.info("✅ QueryMetricsTools 初始化成功, Prometheus URL: {}, Mock模式: {}, 失败降级Mock: {}",
                prometheusBaseUrl, mockEnabled, fallbackMockOnError);
    }
     ...
     */
    @Tool(description = "Query **active/firing alert rules** from Prometheus (/api/v1/alerts)—alert names, descriptions, state. "
            + "This is NOT the same as live node_exporter CPU/RAM/disk/load on the lab host; for that use getLabServerRuntimeSnapshot. "
            + "Use queryPrometheusAlerts when the user asks about firing alerts, alert storms, or alert-level status—not for instant host resource percentages.")
    public String queryPrometheusAlerts() {
        logger.info("开始查询 Prometheus 活动告警, Mock模式: {}, 失败降级Mock: {}", mockEnabled, fallbackMockOnError);
        
        try {
            if (mockEnabled) {
                List<SimplifiedAlert> simplifiedAlerts = buildMockAlerts();
                logger.info("使用 Mock 数据，返回 {} 个模拟告警", simplifiedAlerts.size());
                return buildSuccessJson(simplifiedAlerts,
                        String.format("成功检索到 %d 个活动告警（Mock）", simplifiedAlerts.size()));
            }
            ...
            try {
                ...
                if (!"success".equals(result.getStatus())) {
                    if (fallbackMockOnError) {
                        List<SimplifiedAlert> fallback = buildMockAlerts();
                        String err = result.getError() != null ? result.getError() : "";
                        logger.warn("Prometheus API 状态非 success: {}，已降级为 Mock 告警", result.getStatus());
                        return buildSuccessJson(fallback,
                                String.format("Prometheus 返回状态「%s」，已使用演示告警（%d 条）。%s",
                                        result.getStatus(), fallback.size(),
                                        err.isEmpty() ? "" : "详情: " + err));
                    }
                    return buildErrorResponse("Prometheus API 返回非成功状态: " + result.getStatus(), result.getError());
                }

                List<SimplifiedAlert> simplifiedAlerts = parsePrometheusAlerts(result);
                return buildSuccessJson(simplifiedAlerts,
                        String.format("成功检索到 %d 个活动告警", simplifiedAlerts.size()));
            } catch (Exception inner) {
                if (fallbackMockOnError) {
                    List<SimplifiedAlert> fallback = buildMockAlerts();
                    logger.warn("Prometheus 真实查询失败，已降级为 Mock 告警: {}", inner.getMessage());
                    return buildSuccessJson(fallback,
                            String.format("Prometheus 不可达或解析失败，已使用演示告警（%d 条）。prometheus.fallback-mock-on-error=true。原因: %s",
                                    fallback.size(), inner.getMessage()));
                }
                logger.error("查询 Prometheus 告警失败", inner);
                return buildErrorResponse("查询失败", inner.getMessage());
            }
        } catch (Exception e) {
    ...
    private List<SimplifiedAlert> parsePrometheusAlerts(PrometheusAlertsResult result) {
        AlertsData data = result.getData();
        List<PrometheusAlert> raw = (data == null || data.getAlerts() == null)
        ...
        for (PrometheusAlert alert : raw) {
            ...
            Map<String, String> ann = alert.getAnnotations();
            simplified.setDescription(ann == null ? "" : ann.getOrDefault("description", ""));
    ...
    private String buildSuccessJson(List<SimplifiedAlert> simplifiedAlerts, String message) throws JsonProcessingException {
