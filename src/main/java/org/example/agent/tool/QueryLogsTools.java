...
public class QueryLogsTools {
    ...
    public String getAvailableLogTopics() {
        ...
        try {
            ...
            output.setAvailableRegions(List.of("ap-nanjing", "ap-guangzhou", "ap-shanghai", "ap-beijing", "ap-chengdu"));
            output.setDefaultRegion("ap-nanjing");

            output.setMessage(String.format("共有 %d 个可用的日志主题。建议使用默认地域 'ap-nanjing'（安徽省合肥市就近）或省略 region 参数", topics.size()));
    ...
    private static final List<String> VALID_REGIONS = List.of(
            "ap-nanjing", "ap-guangzhou", "ap-shanghai", "ap-beijing", "ap-chengdu"
    );

    private static final String DEFAULT_REGION = "ap-nanjing";
    ...
    public String queryLogs(
            @ToolParam(description = "地域，可选值: ap-nanjing, ap-guangzhou, ap-shanghai, ap-beijing, ap-chengdu。默认 ap-nanjing（安徽省合肥市就近）") String region,
        ...
        String effectiveRegion = (region == null || region.isBlank()) ? DEFAULT_REGION : region.strip();
        if (!VALID_REGIONS.contains(effectiveRegion)) {
            return buildErrorResponse("无效地域: " + effectiveRegion + "，可选: " + String.join(", ", VALID_REGIONS));
        }
        ...
        try {
            ...
            if (mockEnabled) {
                ...
                logEntries = buildMockLogs(effectiveRegion, logTopic, safeQuery, actualLimit);
            ...
            output.setRegion(effectiveRegion);
