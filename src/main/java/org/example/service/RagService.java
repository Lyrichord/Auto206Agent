...
public class RagService {
    ...
    @Autowired
    private RagRerankProperties ragRerankProperties;
    ...
    public void init() {
        ...
        logger.info("RAG 服务初始化完成，model: {}, topK: {}, rerank: {} ({})",
                model, topK, ragRerankProperties.isEnabled(), ragRerankProperties.getModel());
    }
    ...
    public void queryStream(String question, List<Map<String, String>> history, StreamCallback callback) {
        try {
            ...
            // 1. 向量召回（启用 rerank 时扩大 TopK）+ 可选重排序
            int recallK = ragRerankProperties.recallSize(topK);
            List<VectorSearchService.SearchResult> candidates =
                    vectorSearchService.searchSimilarDocumentsMerged(question, recallK);
            List<VectorSearchService.SearchResult> searchResults =
                    ragRerankService.rerankIfEnabled(question, candidates, topK);
